...
public class FlightDao {
	...
	private static final String DEPARTURE_AND_ARRIVAL_AND_TIME = "select id,departure_city, arrival_city, cost,start_time,end_time, departure_time, arrival_time,stops from flight where departure_city=? and arrival_city=? and departure_time>=? and departure_time<=? and cost>=? and cost<=?" ;
	public FlightDao() {
	}
	...
	public List<Flight> filterFlight(Connection connection){
		...
		try (// Step 2:Create a statement using connection object
			...
			while (rs.next()) {
				...
				String departure_city = rs.getString("departure_city");
				String arrival_city = rs.getString("arrival_city");
				int cost = 90*rs.getInt("cost")/100;
				String start_time = rs.getString("start_time");
				String end_time = rs.getString("end_time");
				String departure_time = rs.getString("departure_time");
				String arrival_time = rs.getString("arrival_time");
	...
	public List<Flight> filterFlightCityTime(String departureCity, String arrivalCity, String startTime, String endTime, String minCost, String maxCost, Connection connection){
		List<Flight> flights = new ArrayList<>();
		// Step 1: Establishing a Connection
		try (// Step 2:Create a statement using connection object
			...
			preparedStatement.setString(5, minCost);
			preparedStatement.setString(6, maxCost);
			System.out.println(preparedStatement);
			ResultSet rs = preparedStatement.executeQuery();
